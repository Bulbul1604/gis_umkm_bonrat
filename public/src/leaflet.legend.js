    ...
    class PolygonSymbol extends GeometricSymbol {
        _drawSymbol() {
            ...
            for (var i = 0; i <= this._legend.sides; i++) {
                var x1 =
                    x0 + r * Math.cos(((a * i + (90 - a / 2)) * Math.PI) / 180);
                var y1 =
                    y0 + r * Math.sin(((a * i + (90 - a / 2)) * Math.PI) / 180);
                if (i == 0) {
                    ctx.moveTo(x1, y1);
                } else {
                    ctx.lineTo(x1, y1);
    ...
    L.Control.Legend = L.Control.extend({
        ...
        _buildLegendItems: function (legendContainer, legend) {
            ...
            if (legend.type === 'image') {
                ...
                legendSymbol = new RectangleSymbol(
                    this,
                    symbolContainer,
                    legend
                );
                ...
                legendSymbol = new PolylineSymbol(
                    this,
                    symbolContainer,
                    legend
                );
